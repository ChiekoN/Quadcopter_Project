...
class DDPG_Agent():
    ...
    def step(self, next_state, reward, done):
        ...
        if len(self.memory) > self.batch_size:
            ...
            experiences = self.memory.sample(batch_size=len(self.memory))
            self.learn(experiences)
    ...
    def act(self, state):
        ...
        # Choose action based on given state and the actor's policy
        action = self.actor_local.model.predict(state)[0]
        self.last_action = list(action + self.noise.sample())
